...
class TestProfiles:
    ...
    def test_dataframe_profile_valid_312(self, test_df):
    ...
    def test_dataframe_time_profile_valid_312(self, cat_df):
    ...
    def test_dataframe_profile_invalid(
        self, non_series_invalid, num_series, cat_like_series
    ):
    ...
    def test_dataframe_profile_html(self, cat_df):
        ...
        assert (
            first_td["style"] == "font-family: monospace, monospace; text-align: left;"
        )
        assert len(tables[3].find_all("tr")) == 3  # 2 deltas + head row

    def test_series_profile_text_valid_numerical_format(self, num_series):
    ...
    def test_series_profile_text_valid_object_format(self, cat_like_series):
    ...
    def test_series_profile_text_valid_time_format(self, cat_df):
    ...
    def test_series_profile_series_dtypes(self):
    ...
    def test_series_profile_singleton(self):
        ...
        series = [
            ...
            pd.Series(
                [pd.Period(start + (dur * x), "M") for x in range(1)]
            ),  # period[M]
            pd.Series(
    ...
    def test_series_profile_complex(self):
    ...
    def test_series_profile_complex_format(self):
        """Profile for complex series should contain and display imaginary component."""
        series = pd.Series([complex(x, x) for x in range(10)])
        profile = php.SeriesProfile(series)
        assert re.findall("mean\\s+[(]4.5[+]4.5j[)]", repr(profile))

    def test_series_profile_invalid(self, non_series_invalid, test_df):
    ...
    def test_series_profile_html(self, cat_df):
    ...
    def test_series_profile_frequency_table(self, test_df):
    ...
    def test_series_profile_time_index_true(self, simple_df):
    ...
    def test_series_profile_time_index_false(self, simple_df):
        """time_index=False does not calculate time diffs
        for Series with DateTimeIndex.
        """
        series = simple_df["category"]
    ...
    def test_series_profile_both_time_index_true(
        self, ts_timeindex
    ):  # pylint: disable=W0621
    ...
    def test_series_profile_frequency_table_invalid(self, test_df):
    ...
    def test_abbreviate_df_invalid_input(self, non_series_invalid):
    ...
    def test_abbreviate_df_valid_output(self, test_df):
    ...
    def test_abbreviate_df_most_plus_least_greater_sum(self, test_df):
    ...
    def test_abbreviate_df_negative_params(self, test_df):
