...
from tests.utils import make_category_data


class TestTimes:
    """Class to test times module."""

    def test_time_diffs(self, simple_df):
    ...
    def test_time_diffs_exception(self):
    ...
    def test_time_diffs_index(self, simple_df):
    ...
    def test_time_diffs_index_exception(self):
    ...
    def test_id_gaps_index(self, ts_timeindex):
    ...
    def test_id_gaps_index_no_gaps(self, ts_timeindex):
        """id_gap_index returns empty Dataframe when threshold exceeds diffs."""
        result = pht.id_gaps_index(
            ts_timeindex, pd.Timedelta(minutes=60, microseconds=1)
        )
        assert len(result) == 0

    def test_id_gaps(self, ts_timeindex):
    ...
    def test_id_gaps_no_gaps(self, ts_timeindex):
        """id_gap returns empty Dataframe when threshold exceeds diffs."""
        result = pht.id_gaps(ts_timeindex, pd.Timedelta(hours=4, microseconds=1))
        assert len(result) == 0

    def test_category_gaps_frequency(self, cat_df):
        """Gaps are calculated correctly for categories
        of varying frequency in Series.
        """
    ...
    def test_category_gaps_no_gaps(self):
    ...
    def test_category_gaps_nulls(self):
        """Nulls should be treated as separate categories
        with correctly calculated gaps.
        """
        start = pd.Timestamp(year=1999, month=1, day=1)
    ...
    def test_category_gaps_timedelta_wrong_type_exception(self, cat_df):
        """Wrong input type for threshold raises exception."""
        with pytest.raises(TypeError) as exc:
            pht.category_gaps(
                cat_df["category"], pd.Timestamp(year=1999, month=1, day=1)
            )
        assert str(pd.Timedelta) in str(exc.value)

    def test_category_gaps_warning(self, cat_df):
